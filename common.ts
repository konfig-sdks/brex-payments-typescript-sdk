...
import { readableStreamToString, BrexPaymentsError, parseIfJson } from "./error";
    ...
    const authenticate = async () => {
        if (configuration.oauthClientId && configuration.oauthClientSecret && configuration.accessToken === undefined) {
            const token = await wrapAxiosRequest(async () => {
                const url = configuration.oauthTokenUrl ?? "https://accounts-api.brex.com/oauth2/default/v1/authorize"
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BrexPaymentsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BrexPaymentsError(e, e.response?.data)
                }
                if (innerError instanceof BrexPaymentsError) {
                    // Got "BrexPaymentsError" from the above try block
                    throw innerError;
                }
